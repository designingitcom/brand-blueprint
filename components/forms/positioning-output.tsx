...
export function PositioningOutput({
  ...
  const [positioningData, setPositioningData] =
    useState<PositioningOutput | null>(null);
  ...
  useEffect(() => {
    const generatePositioning = async () => {
      ...
      const problem =
        responses['1']?.answer || 'Undefined positioning challenge';
      const category = responses['2']?.answer || 'Strategic consulting';
      const transformation =
        responses['4']?.answer || 'Clear market differentiation';
      const identity =
        responses['7']?.answer || 'Strategic positioning experts';
      const contrarian =
        responses['9']?.answer ||
        'Fast strategic clarity over lengthy analysis';
      const onlyPosition =
        responses['12']?.answer ||
        'Only platform delivering real-time positioning';
      const uniqueValue =
        responses['14']?.answer || 'Strategic clarity in days not months';
      ...
      const uniquePosition = Math.max(
        calculateScore('12'),
        calculateScore('7')
      );
      const valueClarity = calculateScore('14');
      const overall = Math.round(
        (problemClarity + segmentFocus + uniquePosition + valueClarity) / 4
      );
      ...
      const positioning: PositioningOutput = {
        ...
        identity: identity,
        values: responses['8']?.answer?.split(',').map(v => v.trim()) || [
          'Quality',
          'Speed',
          'Results',
        ],
      ...
      <div className="space-y-8">
        <div className="text-center py-16">
          ...
          <h2 className="text-2xl font-semibold mb-3">
            Generating Your Strategic Positioning
          </h2>
          <p className="text-muted-foreground max-w-md mx-auto">
            Our AI is analyzing your responses to create a comprehensive
            positioning strategy that differentiates you in the market...
          </p>
    ...
    return (
      <div className="text-center py-16">
        <p className="text-muted-foreground">
          Unable to generate positioning. Please try again.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Based on your responses, we've created a comprehensive positioning
          strategy that differentiates your business and attracts your ideal
          customers.
        </p>
      </div>
            ...
            {[
              {
                ...
                score: positioningData.strength_scores.problem_clarity,
              },
              {
                label: 'Segment Focus',
                score: positioningData.strength_scores.segment_focus,
              },
              {
                label: 'Unique Position',
                score: positioningData.strength_scores.unique_position,
              },
              {
                label: 'Value Clarity',
                score: positioningData.strength_scores.value_clarity,
              },
            ].map((item, idx) => (
          ...
          <div className="text-center pt-4 border-t">
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">
                Overall Positioning Strength
              </p>
              <div
                className={cn(
                  'text-3xl font-bold',
                  getScoreColor(positioningData.strength_scores.overall)
                )}
              >
      ...
      <div className="text-center text-sm text-muted-foreground pt-4">
        <p>
          Generated in{' '}
          {Math.floor(
            (Date.now() - new Date(positioningData.generated_at).getTime()) /
              1000
          )}{' '}
          seconds using AI analysis
        </p>
      </div>
    </div>
  );
