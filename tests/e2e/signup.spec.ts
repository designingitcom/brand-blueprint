...
test.describe('Signup Flow', () => {
  test('should successfully sign up a new user', async ({ page }) => {
    ...
    const successMessage = page.getByText('Please check your email to confirm your account before signing in.');
  ...
  test('should validate required fields', async ({ page }) => {
    ...
    const hasValidationError = await page.getByText(/required|invalid|enter/i).isVisible({ timeout: 2000 }).catch(() => false);
