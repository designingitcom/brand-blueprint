...
export function BusinessSetupWizard({ userId, onComplete, existingBusiness }: BusinessSetupWizardProps) {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    documents: existingBusiness?.documents?.length > 0 ? existingBusiness.documents : [],
  ...
  const updateService = (index: number, field: 'name' | 'url', value: string) => {
    const newServices = [...formData.services];
    newServices[index] = { ...newServices[index], [field]: value };
    updateFormData({ services: newServices });
  };

  const updateCompetitor = (index: number, field: 'url' | 'notes', value: string) => {
  ...
  const addCompetitor = () => {
    updateFormData({ competitors: [...formData.competitors, { url: '', notes: '' }] });
  };
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 3) {
      ...
      const hasValidService = formData.services.some(service => service.name && service.name.trim());
      if (!hasValidService) {
        toast.error('Missing Information', {
  ...
  const saveBusinessBasics = async () => {
    ...
    try {
      const basicsData = {
        ...
        ...(formData.custom_industry && { custom_industry: formData.custom_industry }),
        user_id: userId,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const cleanedCompetitors = formData.competitors.filter(c => c.url || c.notes);

      // Extract documents and conditional enum fields from formData to avoid database errors
      const { documents, annual_revenue, business_type, years_in_business, employee_count, ...formDataWithoutDocuments } = formData;
      ...
      const businessData = {
        ...
        ...(annual_revenue &&
           ['<$1M', '$1-10M', '$10-50M', '$50M+', 'Not disclosed'].includes(annual_revenue) &&
           { annual_revenue }),
        ...(business_type &&
           ['B2B', 'B2C', 'B2B2C', 'Marketplace', 'Non-profit'].includes(business_type) &&
           { business_type }),
        ...(years_in_business &&
           ['<1', '1-3', '3-5', '5-10', '10+'].includes(years_in_business) &&
           { years_in_business }),
        ...(employee_count &&
           ['1-10', '11-50', '51-200', '200-1000', '1000+'].includes(employee_count) &&
           { employee_count })
      };
        ...
        <div className="space-y-8">
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Upload documents like pitch deck, one-pager, business summary, or any other relevant materials
              </p>
              ...
              {isAnalyzing ? (
                <>
                  ...
                  <p className="text-muted-foreground">
                    Our AI is analyzing your business information to provide personalized insights...
                  </p>
                </>
              ) : (
                <>
                  <CheckCircle className="h-12 w-12 mx-auto mb-4 text-green-500" />
                  <p className="text-lg font-medium mb-2">Your business profile is ready!</p>
                  <p className="text-muted-foreground">
