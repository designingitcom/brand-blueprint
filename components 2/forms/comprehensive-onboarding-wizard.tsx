...
type OnboardingPhase = 'business_setup' | 'strategic_foundation' | 'strategic_questions' | 'positioning_output' | 'completed';
...
export function ComprehensiveOnboardingWizard({
  ...
  const [currentPhase, setCurrentPhase] = useState<OnboardingPhase>('business_setup');
  ...
  const [strategicResponses, setStrategicResponses] = useState<Record<string, StrategicQuestionResponse>>({});
  const [positioningData, setPositioningData] = useState<PositioningOutputType | null>(null);
  ...
  const getBusinessSetupDescription = () => {
    switch (businessSetupStep) {
      case 1: return 'Step 1/3: Business Basics';
      case 2: return 'Step 2/3: Structure & Goals';
      case 3: return 'Step 3/3: Services & Intelligence';
      default: return 'Business Information';
    }
  };

  const phases = [
    { key: 'business_setup', title: '1. Business Setup', icon: Target, description: getBusinessSetupDescription() },
    { key: 'strategic_foundation', title: '2. Strategic Foundation', icon: User, description: 'Market segments & personas' },
    { key: 'strategic_questions', title: '3. Strategic Questions', icon: Brain, description: '15 positioning questions' },
    { key: 'positioning_output', title: '4. Positioning Output', icon: Award, description: 'Your strategic positioning' }
  ];
  ...
  const getOverallProgress = () => {
    ...
    totalQuestions = businessSetupTotal + strategicFoundationTotal + strategicQuestionsTotal + positioningOutputTotal;
  ...
  const handleSegmentSelected = async (primary: MarketSegment, secondary?: MarketSegment) => {
  ...
  const handlePersonaSelected = async (persona: BuyerPersona, customizations?: string) => {
  ...
  const handleQuestionsCompleted = async (responses: Record<string, StrategicQuestionResponse>) => {
    setIsLoading(true);
  ...
  const handlePositioningComplete = async (positioning: PositioningOutputType) => {
    setIsLoading(true);
  ...
  const handlePrevious = () => {
    const phases: OnboardingPhase[] = ['business_setup', 'strategic_foundation', 'strategic_questions', 'positioning_output'];
  ...
  const renderCurrentPhase = () => {
    switch (currentPhase) {
          ...
          <div className="space-y-6">
            {businessSetupStep === 1 && (
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  <h2 className="text-base font-medium text-primary">Step 1/3: Business Basics</h2>
            ...
            {businessSetupStep === 2 && (
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  <h2 className="text-base font-medium text-primary">Step 2/3: Structure & Goals</h2>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm text-muted-foreground">
                      <span>Question {businessDetailsSubStep} of 6</span>
                      <span>{Math.round((businessDetailsSubStep / 6) * 100)}%</span>
                    </div>
                    <div className="w-full bg-secondary rounded-full h-2">
                      <div
                        className="bg-primary h-2 rounded-full transition-all duration-300"
                        style={{ width: `${(businessDetailsSubStep / 6) * 100}%` }}
                      />
                    </div>
                  </div>
                </div>
                ...
                {businessDetailsSubStep === 1 && (
                  <div className="space-y-6">
                    <div className="space-y-2 text-center">
                      <h2 className="text-3xl font-bold">What type of business do you run?</h2>
                      <p className="text-muted-foreground">Choose the model that best describes your business</p>
                    </div>
                ...
                {businessDetailsSubStep === 2 && (
                  <div className="space-y-6">
                    <div className="space-y-2 text-center">
                      <h2 className="text-3xl font-bold">How long have you been in business?</h2>
                      <p className="text-muted-foreground">This helps us understand your business maturity</p>
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 max-w-4xl mx-auto">
                      {[
                        { value: '<1', label: '<1 year', description: 'Just starting out' },
                        { value: '1-3', label: '1-3 years', description: 'Early stage' },
                        { value: '3-5', label: '3-5 years', description: 'Growth stage' },
                        { value: '5-10', label: '5-10 years', description: 'Established' },
                        { value: '10+', label: '10+ years', description: 'Mature business' }
                      ].map((years) => (
                        <div
                          key={years.value}
                          onClick={() => {
                            setFormData({ ...formData, yearsInBusiness: years.value });
                        ...
                        >
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground">{years.description}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {businessDetailsSubStep === 3 && (
                  <div className="space-y-6 text-center">
                    <div className="space-y-2">
                      <h2 className="text-3xl font-bold">How many employees do you have?</h2>
                      <p className="text-muted-foreground">Including yourself and any contractors</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 max-w-4xl mx-auto">
                      {[
                        ...
                        { value: '200-1000', label: '200-1000', description: 'Large company' },
                        { value: '1000+', label: '1000+', description: 'Enterprise' }
                      ].map((count) => (
                        <div
                          key={count.value}
                          onClick={() => {
                            setFormData({ ...formData, employeeCount: count.value });
                ...
                {businessDetailsSubStep === 4 && (
                  <div className="space-y-6 text-center">
                    <div className="space-y-2">
                      <h2 className="text-3xl font-bold">What's your business model?</h2>
                      <p className="text-muted-foreground">How do you typically charge for your services?</p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
                      {[
                        { value: 'Subscription', label: 'Subscription', description: 'Recurring monthly/yearly fees' },
                        { value: 'One-time', label: 'One-time', description: 'Project-based pricing' },
                        { value: 'Retainer', label: 'Retainer', description: 'Monthly retainer agreements' },
                        { value: 'Commission', label: 'Commission', description: 'Performance-based fees' }
                      ].map((model) => (
                        <div
                          key={model.value}
                          onClick={() => {
                ...
                {businessDetailsSubStep === 5 && (
                  <div className="space-y-6 text-center">
                    <div className="space-y-2">
                      <h2 className="text-3xl font-bold">What's your average customer lifetime value?</h2>
                      <p className="text-muted-foreground">Total revenue you expect from a typical customer</p>
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 max-w-4xl mx-auto">
                      {[
                        { value: '<$1K', label: '<$1K', description: 'Low-touch customers' },
                        { value: '$1-10K', label: '$1-10K', description: 'Standard customers' },
                        { value: '$10-100K', label: '$10-100K', description: 'Premium customers' },
                        { value: '$100K+', label: '$100K+', description: 'Enterprise clients' },
                        { value: 'Not sure', label: 'Not sure', description: 'Still figuring it out' }
                      ].map((ltv) => (
                        <div
                          key={ltv.value}
                          onClick={() => {
                            setFormData({ ...formData, avgCustomerLTV: ltv.value });
                        ...
                        >
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground">{ltv.description}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {businessDetailsSubStep === 6 && (
                  <div className="space-y-6 text-center">
                    <div className="space-y-2">
                      <h2 className="text-3xl font-bold">What's your primary goal for the next 12 months?</h2>
                      <p className="text-muted-foreground">Choose the most important objective for your business</p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
                      {[
                        { value: 'Find product-market fit', label: 'Find product-market fit', description: 'Validate and refine your offering' },
                        { value: 'Scale what\'s working', label: 'Scale what\'s working', description: 'Grow proven business model' },
                        { value: 'Enter new market', label: 'Enter new market', description: 'Expand to new segments' },
                        { value: 'Improve positioning', label: 'Improve positioning', description: 'Clarify your value proposition' },
                        { value: 'Prepare for funding/exit', label: 'Prepare for funding/exit', description: 'Get ready for investment or sale' }
                      ].map((goal) => (
                        <div
                          key={goal.value}
                          onClick={() => {
                  ...
                  {[1, 2, 3, 4, 5, 6].map((step) => (
                    <div
                      ...
                      className={cn(
                        ...
                        step <= businessDetailsSubStep ? "bg-primary" : "bg-border"
                      )}
                    />
                  ))}
                </div>
            ...
            {businessSetupStep === 3 && (
              <div className="space-y-8">
                {/* Step Header with Progress */}
                <div className="space-y-4">
                  <h2 className="text-base font-medium text-primary">Step 3/3: Services & Intelligence</h2>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm text-muted-foreground">
                ...
                {servicesStep === 3 && (
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Upload any relevant documents (pitch decks, marketing materials, case studies, etc.) to help us understand your business better.
                    </p>
      ...
      case 'positioning_output':
        if (!businessData || !selectedSegment || !selectedPersona || !strategicResponses) return null;
        return (
          <PositioningOutput
            businessData={businessData}
            selectedSegment={selectedSegment}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <div className="text-sm text-muted-foreground">
                  Question {overallProgress.current}/{overallProgress.total} - {overallProgress.percentage}%
                </div>
            ...
            <div className="flex items-center justify-between">
              {phases.map((phase, index) => {
                ...
                const isNavigationAllowed = isPhaseNavigationAllowed(phase.key as OnboardingPhase);
                      ...
                      <div className="relative">
                        <div
                          className={cn(
                            ...
                            isNavigationAllowed ? "cursor-pointer hover:scale-105" : "",
                            isCompleted
                        ...
                        {isCompleted && (
                          <div
                            ...
                            onClick={() => handlePhaseNavigation(phase.key as OnboardingPhase)}
                          >
                    ...
                    <div className="text-center">
                      <p
                        className={cn(
                          ...
                          isNavigationAllowed ? "cursor-pointer hover:text-primary" : "",
