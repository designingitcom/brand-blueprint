...
export function SimpleSegmentSelection({ onSegmentSelected }: SimpleSegmentSelectionProps) {
  ...
  const handlePrimaryChange = (value: string) => {
    ...
    const secondary = secondarySegment ? availableSegments.find(s => s.id === secondarySegment) : undefined;
  ...
  const handleSecondaryChange = (value: string) => {
    ...
    const primary = primarySegment ? availableSegments.find(s => s.id === primarySegment) : null;
    const secondary = (value && value !== 'none') ? availableSegments.find(s => s.id === value) : undefined;
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={secondarySegment} onValueChange={handleSecondaryChange}>
            <SelectTrigger className="max-w-md">
        ...
        <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
          <div className="space-y-2">
            ...
            <p className="text-sm text-blue-700 dark:text-blue-300">
              <strong>Primary:</strong> {availableSegments.find(s => s.id === primarySegment)?.name}
              {secondarySegment && secondarySegment !== 'none' && (
                <>
                  <br />
                  <strong>Secondary:</strong> {availableSegments.find(s => s.id === secondarySegment)?.name}
                </>
              )}
            </p>
          </div>
