...
interface SegmentSelectionProps {
  businessData: any;
  onSegmentSelected: (primary: MarketSegment, secondary?: MarketSegment) => void;
  onPrevious?: () => void;
  onNext?: () => void;
  isLoading?: boolean;
  quickStart?: boolean;
...
export function SegmentSelection({
  ...
  useEffect(() => {
    const generateAISuggestions = async () => {
      ...
      const suggestedSegments = quickStart ? allSegments.slice(0, 3) : allSegments;
  ...
  const handleSegmentSelection = (segmentId: string, type: 'primary' | 'secondary') => {
    ...
    const secondary = newSecondaryId ? segments.find(s => s.id === newSecondaryId) : undefined;
  ...
  const handleChatWithAI = () => {
    ...
    toast.info('AI chat feature coming soon!', {
      description: 'You\'ll be able to discuss segment options with our AI assistant.'
    });
  };
      ...
      <div className="space-y-8">
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            Generating personalized market segments based on your industry, services, and competitors...
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
          <p className="text-sm text-blue-800 dark:text-blue-200">
            <strong>Based on your {businessData?.industry} business serving {businessData?.services?.[0]?.name || 'your services'},</strong>
            <br />
            Select 1-2 market segments. The first selected will be your primary segment.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <Label className="text-lg font-medium">Select segments (1-2 maximum)</Label>
          <Button
            variant="outline"
            size="sm"
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      if (value === 'primary') {
                        ...
                        const remainingPrimary = isPrimary ? undefined : segments.find(s => s.id === primarySegment);
                        const remainingSecondary = isSecondary ? undefined : segments.find(s => s.id === secondarySegment);
                        if (remainingPrimary) {
                          onSegmentSelected(remainingPrimary, remainingSecondary);
                        }
                      }
                    }}
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="primary" disabled={primarySegment !== '' && !isPrimary}>
                        Primary
                      </SelectItem>
                      <SelectItem value="secondary" disabled={secondarySegment !== '' && !isSecondary}>
                        Secondary
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <div className="pr-36">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-xs font-medium text-red-700 dark:text-red-400">Pain Point</p>
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-xs font-medium text-green-700 dark:text-green-400">Opportunity</p>
      ...
      {primarySegment && (
        <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
          <div className="space-y-2">
            <p className="font-medium text-blue-800 dark:text-blue-200">Selected Segments:</p>
            <p className="text-sm text-blue-700 dark:text-blue-300">
              <strong>Primary:</strong> {segments.find(s => s.id === primarySegment)?.name}
              {secondarySegment && (
                <>
                  <br />
                  <strong>Secondary:</strong> {segments.find(s => s.id === secondarySegment)?.name}
                </>
              )}
            </p>
          </div>
