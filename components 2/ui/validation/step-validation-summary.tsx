...
export function StepValidationSummary({
  ...
  return (
    <div className={cn(
      ...
      hasWarnings ? 'border-neutral-300 bg-neutral-50' :
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-medium">
              {stepName} {hasErrors ? 'Validation Issues' : hasWarnings ? 'Warnings' : 'Complete'}
            </h3>
...
export function FormValidationSummary({
  ...
  const totalErrors = stepValidations.reduce((sum, validation) => sum + validation.errors.length, 0);
  const totalWarnings = stepValidations.reduce((sum, validation) => sum + validation.warnings.length, 0);
  const completedSteps = stepValidations.filter(validation => validation.canProceed).length;

  const visibleValidations = showAllSteps
    ? stepValidations
    : stepValidations.filter(validation =>
