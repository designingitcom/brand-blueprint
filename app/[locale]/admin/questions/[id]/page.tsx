...
import { getQuestion, getQuestions, Question } from '@/app/actions/questions';
...
export default function QuestionDetailPage() {
  ...
  useEffect(() => {
    async function loadQuestion() {
      try {
        const [questionResult, questionsResult] = await Promise.all([
        ...
        if (questionResult.success && questionResult.data) {
          ...
          // Load translations - if this is a master question, get its translations
          // If this is a translation, get other translations of the same master
          if (questionsResult.success && questionsResult.data) {
            let translationsList: Question[] = [];

            if (questionResult.data.is_master) {
              // Get all translations of this master question
              translationsList = questionsResult.data.filter(q =>
                q.parent_question_id === questionResult.data.id
              );
            } else if (questionResult.data.parent_question_id) {
              // Get the master question and all other translations
              const masterQuestion = questionsResult.data.find(q =>
                q.id === questionResult.data.parent_question_id
              );
              const otherTranslations = questionsResult.data.filter(q =>
                q.parent_question_id === questionResult.data.parent_question_id &&
                q.id !== questionResult.data.id
              );
              translationsList = masterQuestion ? [masterQuestion, ...otherTranslations] : otherTranslations;
            }
  ...
  const handleTranslationUpdate = (newTranslation: Question) => {
    ...
    >
            ...
            <div className="p-4 bg-primary/5 rounded-lg border-l-4 border-primary">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium text-sm">Current Question</div>
                    <div className="text-xs text-muted-foreground">
                      Language: {(question.content_language || 'en').toUpperCase()}
                      {question.is_master && ' (Master)'}
                      {!question.is_master && ' (Translation)'}
                    </div>
                  </div>
                </div>
            ...
            {translations.length > 0 ? (
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  {question.is_master
                    ? `This master question has ${translations.length} translation(s):`
                    : `Other language versions of this question:`}
                </p>
                <div className="space-y-3">
                  {translations.map((translation) => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <Badge
                            variant={translation.is_master ? "default" : "secondary"}
                            className="text-xs"
                          >
                          ...
                          <Badge
                            ...
                            className={cn(
                              "text-xs",
                              translation.is_master
                                ? "bg-blue-500/10 text-blue-500 border-blue-500/20"
                                : "bg-orange-500/10 text-orange-500 border-orange-500/20"
                            )}
                          >
                            {translation.is_master ? 'Master' : 'Translation'}
                          </Badge>
                          {translation.module && (
                            <Badge variant="outline" className="text-xs">
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  {question.is_master
                    ? 'This master question hasn\'t been translated to other languages yet. Click "Add Translation" to create language versions.'
                    : 'This translation doesn\'t have related language versions available.'}
                </p>
                {question.is_master && (
                  <Button
                    variant="outline"
      ...
      {question?.is_master && (
        <CloneTranslateModal
          ...
          onSuccess={handleTranslationUpdate}
        />
      )}
    </DashboardLayout>
  );
